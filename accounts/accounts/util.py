...
def send_account_activation_email(email):
    ...
    base_url = "%s://%s%s/activate" % (settings.HTTP_PROTOCOL, domain_url, user_api_path)
...
def send_reset_password_email(email):
    ...
    base_url = "%s://%s%s/confirm_reset_password" % (settings.HTTP_PROTOCOL, domain_url, user_api_path)
    confirm_link = "%s?auth_code=%s" % (base_url, urllib.parse.quote_plus(encrypted_data))
...
def notify_security_email(email, ip_address, token, request, accesslog_pk):
    ...
    base_url = "%s://%s%s/ban" % (settings.HTTP_PROTOCOL, domain_url, user_api_path)
    ban_url = "%s?auth_code=%s" % (base_url, urllib.parse.quote_plus(encrypted_data))

    base_url = "%s://%s%s/reset_password" % (settings.HTTP_PROTOCOL, domain_url, user_api_path)
